...
| **导出方式** | `常规导出` | `常规导出` = 内存拼接输出 images/audio（连接 CreateVideo 使用）；`流式导出` = 从分段缓存流式渲染成片并写 mp4（防 OOM），峰值内存恒定（≈6GB）；流式导出下 images/audio 输出为空，成片路径见 `video_path` |

- **二采由接线决定**：Refine 节点接上就跑、拔掉就停，没有独立开关。选了 `refine_only` 但未接 Refine 时，控件标签会标 ⚠ 提醒（排队后后端也会报错）。

> **升级提示**：旧工作流里的 `stream_export` / `rerun_mode` / 「一采」「二采」「流式导出」「正常导出」
> 开关已移除，加载旧图后这些值会被丢弃、运行模式重置为 `full`（完整流程）、导出方式重置为
> `常规导出`，请按需重新选择。
...
已含音轨（视频+音频），路径见 `video_path`。**用法**：导出方式选「流式导出」→ 正常排队（无需连接 CreateVideo）→
...
> 此时跳过采样期增量导出，统一由缓存逐段渲染。也正因如此，「只跑导出」+「分段导出」可选「流式导出」
...
**只跑导出**：已完整跑过一遍后，把**运行模式改为 `export_only`**（流式或常规导出均可），即不采样、不二采，
仅从分段缓存重新渲染成片——适合调整导出参数（如换「导出方式」、改导出模式）后直接出片，零采样耗时。
